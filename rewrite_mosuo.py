...
export default function MosuoCulturePage() {
  ...
  return (
    <ImmersivePage bgImage="/images/lugu-hero.jpg" className="page-fade-in pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="mb-6">
        <div className="text-amber-400 text-sm font-bold tracking-wider uppercase mb-1 drop-shadow-md text-shadow-sm">Mosuo Culture</div>
      ...
      {tips.length > 0 && (
        <CardComponent variant="glass" className="mb-6">
          ...
          <ul className="text-sm text-white/80 mt-0 mb-0 pl-5 space-y-2 list-disc marker:text-cyan-400">
            {tips.map((item, idx) => (
...
with open("frontend/src/pages/MosuoCulturePage.jsx", "w", encoding="utf-8") as f:
    f.write(content)

print("Mosuo page done")
