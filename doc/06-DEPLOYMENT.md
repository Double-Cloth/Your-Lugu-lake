...
- `PASSWORD_TRANSPORT_ALLOW_PLAINTEXT_FALLBACK`
- `SESSION_COOKIE_SECURE`, `SESSION_COOKIE_SAMESITE`, `SESSION_COOKIE_DOMAIN`
...
- `APP_ENV=production`
- `ALLOWED_HOSTS` 配置真实域名（不要 `*`）
- `SESSION_COOKIE_SECURE=true`（必须 HTTPS）
- `PASSWORD_TRANSPORT_ALLOW_PLAINTEXT_FALLBACK=false`
- `PASSWORD_TRANSPORT_PRIVATE_KEY_PEM` 使用安全密钥管理注入
