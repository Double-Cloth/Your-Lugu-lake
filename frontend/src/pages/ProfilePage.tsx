import { useEffect, useMemo, useRef, useState, ReactNode, ChangeEvent } from "react";
...
// Helper functions
function toRecordList(footprints: Footprint[], locationMap: Record<number, Location>): Record[] {
  return footprints.map((item) => ({
    ...
    photoList: Array.isArray(item.photo_urls) ? item.photo_urls : item.photo_url ? [item.photo_url] : [],
  }));
}

function formatTravelProfile(profile: TravelProfile | null | undefined): string {
...
function normalizePosterTrackPoints(trackPoints: any): TrackPoint[] {
...
function drawTrackPreview(
  ...
  ctx.save();
  ctx.beginPath();
  (ctx as any).roundRect(x, y, width, height, panelRadius);
  ...
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
  (ctx as any).shadowColor = "rgba(54, 196, 239, 0.6)";
...
export default function ProfilePage(): React.ReactElement {
  ...
  const { username: usernameFromPath } = useParams<URLParams>();
  const posterBackgroundUrl = "/images/lugu-hero.png";
  const [username, setUsername] = useState<string>("");
  const [password, setPassword] = useState<string>("");
  const [confirmPassword, setConfirmPassword] = useState<string>("");
  const [authMode, setAuthMode] = useState<"login" | "register">("login");
  const [loggedIn, setLoggedIn] = useState<boolean>(hasUserSession());
  ...
  const [savedRoutesLoading, setSavedRoutesLoading] = useState<boolean>(false);
  const [deletingRouteId, setDeletingRouteId] = useState<number | null>(null);
  const [quickRefreshing, setQuickRefreshing] = useState<boolean>(false);

  const [posterVisible, setPosterVisible] = useState<boolean>(false);
  const [posterTrackPoints, setPosterTrackPoints] = useState<TrackPoint[]>([]);
  ...
  async function loadRecords(): Promise<void> {
    ...
    try {
      const [footprints, locations, routes, trackingState] = await Promise.all([
        ...
        fetchLocations(),
        fetchMyRoutes(token),
        fetchTrackingState().catch(() => null as any),
      ]);
      ...
      const list: Record[] = Array.isArray(footprints) ? toRecordList(footprints, locationMap) : [];
      setRecords(list);
  ...
  async function handleQuickRefresh(): Promise<void> {
  ...
  async function handleDeleteRoute(routeId: number): Promise<void> {
    const token = getUserToken();
    if (!token) {
      Toast.show({ content: "请先登录" });
      return;
  ...
  async function drawPoster(): Promise<void> {
    ...
    ctx.fillText(
      `打卡 ${records.length} 次  ·  探索 ${uniqueSpots} 个景点  ·  路线 ${savedRoutes.length} 条`,
      56,
      510
    );
          ...
          <div className="flex flex-col gap-4">
            ...
            <div
              className={`overflow-hidden transition-all duration-300 ease-in-out ${
                authMode === "register" ? "max-h-20 opacity-100 mt-0" : "max-h-0 opacity-0 -mt-4"
              }`}
            >
              <GlassInput
                icon={<LockOutline />}
  ...
  const renderAuth = (): ReactNode => (
    <div className="animate-[fadeIn_0.4s_ease-out] mx-auto w-full pt-4 pb-4">
      <CardComponent
        variant="immersive"
        className="relative overflow-hidden mb-6 border-white/20 p-6 flex-shrink-0"
      >
        ...
        <div className="profile-function-section">
          ...
          <div className="profile-function-grid profile-function-grid-tool">
            ...
            >
              <div className="profile-function-icon-wrap">
                <svg
                  ...
                  className={`text-emerald-300 ${
                    quickRefreshing || recordLoading || savedRoutesLoading ? "animate-spin" : ""
                  }`}
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
              ...
              <span className="profile-function-text">
                {quickRefreshing || recordLoading || savedRoutesLoading ? "刷新中" : "刷新数据"}
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="mb-6" ref={accountSectionRef}>
        ...
        <CardComponent variant="immersive" className="p-4 space-y-4 shrink-0">
          ...
          <div className="flex gap-3 pt-2">
            ...
            {editMode && (
              <ButtonComponent variant="primary" className="flex-1" onClick={() => void saveProfile()}>
                保存
              </ButtonComponent>
            )}
          </div>
            ...
            <div className="relative border-l-2 border-white/10 ml-3 space-y-6">
              {records.map((item) => (
                <div key={item.id} className="relative pl-5">
                  ...
                  <div className="text-sm text-white/70 leading-relaxed bg-white/5 p-2 rounded-lg mt-2 italic shadow-inner">
                    "{item.mood}"
                  </div>
                </div>
              ))}
            ...
            <EmptyStateComponent
              icon={
                <LucideIcon
                  name="Map"
                  size={48}
                  className="text-white/40 mb-3 mx-auto"
                />
              }
              title="暂无足迹"
                  ...
                  <div key={`route-${item.id}`} className="rounded-2xl border border-white/15 bg-white/5 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-white/55 mt-1">
                          保存时间：{new Date(item.created_at || "").toLocaleString()}
                        </div>
                        {templateTitle ? (
                    ...
                    {timeline.length > 0 ? (
                      <div className="mt-3 space-y-2">
                        {timeline.map((stop: any, idx: number) => (
                    ...
                    <ButtonComponent
                      ...
                      onClick={() =>
                        navigate(withUserSessionPath("/home"), {
                          state: {
                            openPanel: "culture",
                            cultureSeed: {
                              route_id: item.id,
        ...
        <div className="p-6 bg-slate-900/95 backdrop-blur-3xl rounded-t-3xl border-t border-white/20 pb-10">
          ...
          <div className="poster-preview-wrap flex justify-center mb-6">
            <canvas
              ...
              width={720}
              height={1080}
              className="profile-poster-canvas rounded-2xl border border-white/20 shadow-2xl max-w-full"
            />
          </div>

          <div className="flex gap-3">
            <ButtonComponent variant="secondary" className="flex-1" onClick={() => void drawPoster()}>
              重新生成
            </ButtonComponent>
            <ButtonComponent
              variant="primary"
              className="flex-1 shadow-lg shadow-amber-500/30"
