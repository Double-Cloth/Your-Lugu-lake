...
type EnrichedItem = FootprintItem & {
...
export default function ScrollPage(): JSX.Element {
	...
	const [copyingSummary, setCopyingSummary] = useState<boolean>(false);
	const [footprints, setFootprints] = useState<FootprintItem[]>([]);
	const [locationMap, setLocationMap] = useState<Record<number, LocationItem>>({});
	...
	async function loadScroll(): Promise<void> {
		setLoading(true);
		try {
			...
			(Array.isArray(locations) ? locations : []).forEach((loc: LocationItem) => {
				map[loc.id] = loc;
			});

			setLocationMap(map);
			setFootprints(Array.isArray(myFootprints) ? (myFootprints as FootprintItem[]) : []);
		} catch (error: any) {
	...
	async function snapshotScroll(): Promise<HTMLCanvasElement> {
					...
					{filtered.length === 0 ? (
						...
						<div className="scroll-timeline-list">
							{filtered.map((item, idx) => (
								<article key={item.id} className="scroll-timeline-item">
									...
									<div className="scroll-timeline-content">
										<div className="scroll-timeline-head">
											<div>
												<div className="scroll-timeline-time">{item.checkInTime ? formatDateTime(item.checkInTime) : "时间未知"}</div>
