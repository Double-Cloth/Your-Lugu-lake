...
type CardVariant = "default" | "glass" | "neon" | "immersive";
...
type ButtonVariant = "primary" | "immersive" | "secondary" | "danger";
...
type GlassInputProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, "onChange" | "value" | "type"> & {
  className?: string;
...
export const ImmersivePage = ({ bgImage, children, className = "", style }: PageProps) => {
...
export const ReadingGlassCard = ({ children, className = "" }: { children: ReactNode; className?: string }) => {
  return (
...
export const CardComponent = ({
  ...
  const variants: Record<CardVariant, string> = {
    ...
    immersive: "backdrop-blur-md bg-white/10 border border-white/20 shadow-xl text-white",
  };
...
export const ButtonComponent = ({
  ...
  const variants: Record<ButtonVariant, string> = {
    ...
    immersive: "bg-white/90 hover:bg-white text-slate-900 border-none shadow-[0_0_20px_rgba(255,255,255,0.15)]",
...
export const SkeletonComponent = ({ className = "" }: SkeletonProps) => (
  <div className={`animate-pulse bg-white/20 rounded-xl ${className}`}></div>
);

export const EmptyStateComponent = ({
  icon = <LucideIcon name="Frown" size={48} className="text-white/40 mb-3 mx-auto" strokeWidth={1.5} />,
  title = "暂无数据",
  description = "",
  className = "",
}: EmptyStateProps) => (
...
export const GlassInput = forwardRef<HTMLInputElement, GlassInputProps>(function GlassInput(
  ...
  return (
    <div className={`flex items-center rounded-xl transition-all shadow-inner bg-white/10 border border-white/20 backdrop-blur-md px-4 py-2.5 focus-within:border-orange-400 focus-within:ring-2 focus-within:ring-orange-400/60 focus-within:shadow-lg focus-within:shadow-orange-400/20 ${wrapperClassName}`}>
      {icon && (
