...
export default function LocationDetailPage() {
  ...
  const handleBack = () => {
    const fromPanel = locationState.state?.fromPanel;
  ...
  return (
    <ImmersivePage bgImage={bgImage} className="page-fade-in pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="mb-6">
        <div className="text-cyan-400 text-sm font-bold tracking-wider uppercase mb-1 drop-shadow-md text-shadow-sm">Location Detail</div>
      ...
      <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
        <div className="flex items-center justify-between mb-3 border-b border-white/10 pb-3">
          <span className="px-3 py-1 bg-cyan-500/20 text-cyan-200 rounded-full text-xs font-medium border border-cyan-500/30">
            {item.category || "景点"}
          </span>
          <span className="text-white/50 text-xs flex items-center">
            <LucideIcon name="MapPin" size={12} className="mr-1" />
        ...
        <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
          ...
          <div className="flex flex-wrap gap-2">
            {details.highlights.map((highlight, idx) => (
              <span key={idx} className="px-3 py-1.5 bg-white/10 backdrop-blur border border-white/20 text-white rounded-full text-sm shadow-sm transition hover:bg-white/20">
                {highlight}
              </span>
            ))}
          </div>
      ...
      <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
        <h2 className="text-lg font-bold text-white mb-4 flex items-center">
          <LucideIcon name="Image" size={18} className="mr-2 text-cyan-400" />
        ...
        {galleryImages.length > 0 ? (
          ...
          <div className="text-center py-6 bg-white/5 rounded-xl border border-white/10">
            <LucideIcon name="ImageOff" size={32} className="mx-auto text-white/30 mb-2" />
      ...
      {(details.bestSeasonToVisit || details.recommendedDuration) && (
        <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center">
            <LucideIcon name="Compass" size={18} className="mr-2 text-cyan-400" />
      ...
      {(location.province || location.city || location.address) && (
        <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center">
            <LucideIcon name="Map" size={18} className="mr-2 text-cyan-400" />
            {sections.locationTitle || "位置信息"}
          </h2>
          <div className="space-y-2 text-sm text-white/80">
            {location.province && (
              <div className="flex items-center">
                <div className="w-[70px] flex items-center text-white/60"><LucideIcon name="MapPin" size={14} className="mr-1" /> 省份</div>
                <div>{location.province}</div>
              </div>
            )}
            {location.city && (
              <div className="flex items-center">
                <div className="w-[70px] flex items-center text-white/60"><LucideIcon name="Building2" size={14} className="mr-1" /> 城市</div>
                <div>{location.city} {location.district}</div>
              </div>
            )}
            {location.address && (
              <div className="flex items-start mt-3 pt-3 border-t border-white/10">
                <div className="w-[70px] flex items-center text-white/60"><LucideIcon name="Navigation" size={14} className="mr-1" /> 地址</div>
      ...
      {Object.keys(transportation).length > 0 && (
        <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center">
            <LucideIcon name="CarFront" size={18} className="mr-2 text-cyan-400" />
            {sections.transportationTitle || "交通方式"}
          </h2>
          <div className="space-y-4 text-sm text-white/80">
            {transportation.byAir && (
              <div>
                <div className="flex items-center text-white font-medium mb-1"><LucideIcon name="Plane" size={16} className="mr-1.5 text-cyan-300" /> 飞机</div>
                <div className="pl-6">{transportation.byAir}</div>
              </div>
            )}
            {transportation.byTrain && (
              <div>
                <div className="flex items-center text-white font-medium mb-1"><LucideIcon name="Train" size={16} className="mr-1.5 text-cyan-300" /> 火车</div>
                <div className="pl-6">{transportation.byTrain}</div>
              </div>
            )}
            {transportation.byBus && (
              <div>
                <div className="flex items-center text-white font-medium mb-1"><LucideIcon name="Bus" size={16} className="mr-1.5 text-cyan-300" /> 大巴</div>
                <div className="pl-6">{transportation.byBus}</div>
              </div>
            )}
            {transportation.byBoat && (
              <div>
                <div className="flex items-center text-white font-medium mb-1"><LucideIcon name="Ship" size={16} className="mr-1.5 text-cyan-300" /> 游船</div>
      ...
      {Object.keys(facilities).length > 0 && (
        <CardComponent variant="glass" className="mb-4 backdrop-blur-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center">
            <LucideIcon name="Coffee" size={18} className="mr-2 text-cyan-400" />
            ...
            {facilities.parking !== undefined && (
              <div className="flex items-center bg-white/5 p-2 rounded-lg border border-white/5">
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center mr-2">
                  <LucideIcon name="ParkingSquare" size={16} className="text-white" />
                </div>
                <span className="flex-1 text-white/80">停车场</span>
                {facilities.parking ? <LucideIcon name="CheckCircle2" size={16} className="text-green-400" /> : <LucideIcon name="XCircle" size={16} className="text-white/30" />}
              </div>
            )}
            {facilities.restroom !== undefined && (
              <div className="flex items-center bg-white/5 p-2 rounded-lg border border-white/5">
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center mr-2">
                  <LucideIcon name="Users" size={16} className="text-white" />
                </div>
                <span className="flex-1 text-white/80">卫生间</span>
                {facilities.restroom ? <LucideIcon name="CheckCircle2" size={16} className="text-green-400" /> : <LucideIcon name="XCircle" size={16} className="text-white/30" />}
              </div>
            )}
            {facilities.foodAndDrink !== undefined && (
              <div className="flex items-center bg-white/5 p-2 rounded-lg border border-white/5">
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center mr-2">
                  <LucideIcon name="Utensils" size={16} className="text-white" />
                </div>
                <span className="flex-1 text-white/80">餐饮</span>
                {facilities.foodAndDrink ? <LucideIcon name="CheckCircle2" size={16} className="text-green-400" /> : <LucideIcon name="XCircle" size={16} className="text-white/30" />}
              </div>
            )}
            {facilities.accommodation !== undefined && (
              <div className="flex items-center bg-white/5 p-2 rounded-lg border border-white/5">
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center mr-2">
                  <LucideIcon name="BedDouble" size={16} className="text-white" />
                </div>
                <span className="flex-1 text-white/80">住宿</span>
                {facilities.accommodation ? <LucideIcon name="CheckCircle2" size={16} className="text-green-400" /> : <LucideIcon name="XCircle" size={16} className="text-white/30" />}
              </div>
            )}
            {facilities.medicalService !== undefined && (
              <div className="flex items-center bg-white/5 p-2 rounded-lg border border-white/5">
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center mr-2">
                  <LucideIcon name="Stethoscope" size={16} className="text-white" />
                </div>
                <span className="flex-1 text-white/80">医疗</span>
                {facilities.medicalService ? <LucideIcon name="CheckCircle2" size={16} className="text-green-400" /> : <LucideIcon name="XCircle" size={16} className="text-white/30" />}
              </div>
            )}
          </div>
        </CardComponent>
      ...
      {ticketInfo.price && (
        <CardComponent variant="glass" className="mb-6 backdrop-blur-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center">
            <LucideIcon name="Ticket" size={18} className="mr-2 text-cyan-400" />
            {sections.ticketInfoTitle || "票务信息"}
          </h2>
          <div className="space-y-3 text-sm text-white/80">
            <div className="flex items-center">
              <span className="font-semibold text-white w-[70px]">门票：</span>
              <span className="text-cyan-300 font-bold text-lg">{ticketInfo.price}</span>
            </div>
...
with open("frontend/src/pages/LocationDetailPage.jsx", "w", encoding="utf-8") as f:
    f.write(content)

print("Location detail page done")
